...
function cds_display_single_car($post_id) {
	...
	$vehicle_price = money_format('$%.0n', $car['price'], thousands_sep);//RS: remove trailing zeros
					...
					<ul>';
						...
						if (isset($vehicle_details['exterior_color'])) {
							...
							if (!empty($vehicle_details['exterior_color'])) {
								...
								$html .='<li><label>'.__('Color:', 'car-demon-shortcode').'</label> '.$vehicle_details['exterior_color'];

								if (isset($vehicle_details['interior_color'])) {

									if (!empty($vehicle_details['interior_color'])) {

										$html .= '/'.$vehicle_details['interior_color'].'</li>';
...
//				if ($main_guid != $guid) {
					...
					$photo_array = '<div><img id="slide'.$cnt.'" class="kar_thumbs img_slide" src="'.trim($guid).'" /></div>';
...
//					$photo_array = '<a href="#mainpic"></a><img id="slide'.$cnt.'" class="kar_thumbs img_slide" style="cursor:pointer"'.$popup_imgs.' onClick=\'MM_swapImage("'.$car_title_pro.'_pic","","'.trim($thumbnail).'",1);\' src="'.trim($thumbnail).'" />';



					$photo_array = '<div class=""><img id="slide'.$cnt.'" class="kar_thumbs img_slide" src="'.trim($thumbnail).'" /></div>';
